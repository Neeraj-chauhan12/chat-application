    ...
    const getMessages = async () => {
      if (user) {
        const response = await axios.get(
          `${BACKEND_URL}/api/message/messages/${user._id}`,
          {
      ...
      {messages.map((message) => {
        ...
        return (
          <div
            key={message._id}
            ref={lastMsgRef}
            className={`h-[5%] w-full my-2 flex ${
              isCurrentUser ? "justify-end" : "justify-start"
            }`}
          >
            <div
              className={`h-full max-w-[50%] rounded-t-2xl ${
                isCurrentUser
                  ? "rounded-l-2xl bg-green-400"
                  : "rounded-r-2xl bg-blue-400"
              } text-white px-5 py-2 flex items-center justify-start`}
            >
