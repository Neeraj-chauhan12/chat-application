      ...
      <div className="users h-[85%] w-full overflow-y-scroll">
        {filteredUsers.map((user) => {
          ...
          return (
            <div
              key={user._id}
              className={`h-[10%] w-full flex items-center justify-start px-3 cursor-pointer ${
            ...
            >
              <div className="h-10 w-10 relative rounded-full bg-gray-500 flex items-center justify-center text-white mr-3">
                {isOnline && (
                  <div className="h-3 w-3 bg-green-500 absolute rounded-full top-0 right-0 border-2 border-white"></div>
                )}
